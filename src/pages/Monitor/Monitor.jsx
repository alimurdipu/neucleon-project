...
import { FaDownload, FaFilter, FaKey, FaList } from "react-icons/fa";
...
const Monitor = () => {
  ...
  const dataArray = Array(2).fill(undefined).map(
    (_, index) => ({
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-4 gap-2">
        {monitorButton.map((button, index) => (
          <div key={index} className="bg-dark-gray rounded-sm p-1">
            <div className="flex items-center gap-2 w-12 m-auto">
              <p className="text-info text-xl">{button.icon}</p>
              <p className="text-secondary text-center">{button.number}</p>
            </div>
            <p className="text-secondary text-center">{button.name}</p>
          </div>
        ))}
      </div>
      {/* button end */}
